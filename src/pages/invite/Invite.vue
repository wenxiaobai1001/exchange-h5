  ...
  <div class="invite">
    <div class="header">
      <div class="invite-title">
        <div class="my-link-text">{{$t('invite.myinvitelink')}}: <a class="copy-button" v-clipboard:copy="myInfo.inviteLink" v-clipboard:success="onCopy" v-clipboard:error="onError" href="javascript:;">{{$t('invite.copy')}}</a></div>
      ...
      <div class="invite-detail">
        <div class="item">
          <p class="i-v">{{myInfo.levelone}}55</p>
    ...
    <div class="main">
      <div class="invite-content">
        ...
        <div class="content">
          ...
          <div style="font-size: 12px;padding: 10px;width: 100%;word-break: break-word;margin-bottom: 10px;border: 1px solid #c8c8c8;">
      ...
      <div class="promotion-tools">
        ...
        <div class="tools">
          <div class="item" @click="showCardItem">
              <img src="../../assets/images/promotion/promotioncard.png" />
          ...
          <div class="item" @click="useInviteImage">
              <img src="../../assets/images/promotion/invitebg.png" />
              <p class="title">{{$t('invite.pt_invite_title')}}</p>
              <p class="desc">{{$t('invite.pt_invite_desc')}}</p>
          </div>
          <div class="item more">
            <p style="color:#c2c2c2;font-size:18px;font-weight: bold;">{{$t('invite.pt_more')}}</p>
          </div>
          <div class="item">
      ...
      <div class="invite-rank-tabmenu">
        <span @click="ShowActiveTop(1)" :class="{'active': isActiveTop==1}">Commission Top 20</span><span @click="ShowActiveTop(2)" :class="{'active': isActiveTop==2}">Promotions Top 20</span>
      </div>
      <div class="invite-rank" v-if="isActiveTop==1">
        <h3>{{$t('invite.totalcommissionrank')}} TOP 20 {{$t('invite.rankall')}}</h3>
        <Row :gutter="80">
          <Col span="24">
            <p class="ranktitle"><span>{{$t('invite.laststastic')}}: {{lastUpdate}}</span></p>
            <Table :no-data-text="$t('common.nodata')" class="rank-table" :columns="columns" :data="topRewardRank"></Table>
            <p class="rank-tips">* {{$t('invite.ranktip1')}}</p>
          </Col>
        </Row>
      </div>
      <div class="invite-rank" v-if="isActiveTop==2">
        <h3>{{$t('invite.promotioncount')}} TOP 20 {{$t('invite.rankall')}}</h3>
        <Row :gutter="80">
          <Col span="24">
            <p class="ranktitle"><span>{{$t('invite.laststastic')}}: {{lastUpdate}}</span></p>
            <Table :no-data-text="$t('common.nodata')" class="rank-table" :columns="columns" :data="topInviteCountRank"></Table>
      ...
      <div class="invite-rank" style="display:none;">
        <Row :gutter="80">
          <Col span="12">
            ...
            <Table :no-data-text="$t('common.nodata')" class="rank-table" :columns="columns1" :data="topRankWeek"></Table>
            <p class="rank-tips">* {{$t('invite.ranktip2')}}</p>
          </Col>
          <Col span="12">
            ...
            <Table :no-data-text="$t('common.nodata')" class="rank-table" :columns="columns1" :data="topRankMonth"></Table>
            <p class="rank-tips">* {{$t('invite.ranktip2')}}</p>
          </Col>
        </Row>
      </div>
    ...
    <Drawer :title="$t('invite.pt_title')" :closable="true" v-model="showCardModal" width="100">
      <div class="ptcard-header">
        <img class="card-img" src="../../assets/images/promotion/promotioncard.png" />
        <div class="desc">
    ...
    <Drawer :title="promotionTitle" :closable="true" v-model="showPromotionModal" width="100">
      <div style="position:relative;width: 318px;" id="promotionImage" ref="promotionImage">
        <img style="width:100%;display:block;" src="../../assets/images/promotion/promotionbg1.png" />
      ...
      <p style="text-align:center;font-size:14px;color:#888;margin-top: 10px;">{{$t('invite.imagetips')}}</p>
    ...
    <Drawer :title="$t('invite.context_title3')" :closable="true" v-model="showInviteImageModal" width="100">
      <div style="position:relative;width: 100%;display: flex; justify-content: center;" id="inviteImage" ref="inviteImage">
        <img style="width:85%;display:block;" src="../../assets/images/promotion/invitebg1.png" />
        <div class="qr-code" style="background:#FFF;position:absolute;right: 0; left: 0; bottom: 0; top: 110px; width: 100px;margin: auto;border-radius: 5px;height: 100px;">
...
export default {
  ...
  methods: {
    ShowActiveTop(isActiveTop){
      this.isActiveTop = isActiveTop;
    },
    init() {
    ...
    saveInviteImage(){
      this.save("inviteImage", "Invitation Image");
      this.saveImageLoading = true;
    },
    saveImage(){
      this.save("promotionImage", "Promotion partner image");
      this.saveImageLoading = true;
    },
    ...
    getMemberInfo(){
      ...
      // 获取邀请人数
      this.$http.post(this.host + this.api.uc.memberInfo, {})
        .then(response => {
          var resp = response.body;
          if (resp.code == 0) {
      ...
      // 获取个人佣金信息
      this.$http.post(this.host + this.api.uc.mypromotion, {})
        .then(response => {
          var resp = response.body;
          if (resp.code == 0) {
    ...
    getFreeCard: function(){
      this.$http.post(this.host + this.api.uc.getfreecard, {})
        .then(response => {
          var resp = response.body;
          if (resp.code == 0) {
            this.$Notice.success({
...
.invite {
  ...
  .main {
    ...
    .invite-rank{
      ...
      .rank-tips{
        text-align:justify;color:#000;font-size:13px;margin-top:10px;
      }
      .rank-table{
...
.ptcard-header{
  display:flex;flex-direction:column;justify-content:flex-start;margin-top:20px;
  .card-img{
    width:260px;height:140px;border-radius:5px;
  }
  .desc{
    height:130px;width:100%;color:#000;
    .title{
