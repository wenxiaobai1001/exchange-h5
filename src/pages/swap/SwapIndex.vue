<template>
  <div class="container swap" :class="skin">
    <div class="page-exchange-title" style="background-color:#000000;">
      <span>
        <img src="@/assets/img/lines.png" @click="()=>{this.isShowSwapMenu=true;}">
      </span>
      <span>{{currentCoin.coin}}<small>/{{currentCoin.base}}</small></span>
      <span>
      </span>
    </div>
    <ContractTab contractUrl="/swap"></ContractTab>
    <div class="main">
      <div class="tab-menu">
        <span @click="changeTabMenu(1)" :class="{'isActiveMenu':isActiveMenu==1}">K-line</span><span @click="changeTabMenu(2)" :class="{'isActiveMenu':isActiveMenu==2}">Transaction</span><span @click="changeTabMenu(3)" :class="{'isActiveMenu':isActiveMenu==3}">Trade history</span>
      </div>

      <div v-if="isActiveMenu==1" class="left" style="position:relative;">
        
        <div class="swap-menu right" v-if="isShowSwapMenu">
          <div class="coin-menu-close">
            <img src="../../assets/img/white-back.png" @click="()=>{this.isShowSwapMenu=false;}"/>
          </div>
          <div class="coin-menu">
        ...
        <div class="center">
          ...
          <div class="handler handler-line">
          ...
          <div class="imgtable">
            <div id="kline_container" :class="{hiddenkline:currentImgTable==='s'}"></div>
            <DepthGraph :class="{hiddenkline:currentImgTable==='k'}" ref="depthGraph"></DepthGraph>
          </div>
        </div>
        ...
        <div class="plate-nowprice">
          ...
          <span class="price-cny"> ≈ {{currentCoin.usdRate*CNYRate | toFixed(2)}} (THB)</span>
        </div>
        <!-- 盘口：买 -->
        <Table :no-data-text="$t('common.nodata')" v-show="selectedPlate!='sell'" @on-current-change="sellPlate" highlight-row class="buy_table" :class="{hiddenkline:selectedPlate==='all'}" :columns="plate.columns" :data="plate.bidRows"></Table>

      </div>

      <!-- 盘口 -->
      <div v-if="isActiveMenu==2" class="left plate-wrap" style="position:relative;">
        
        <!-- 我的合约账户 -->
        <div class="order" style="margin-top: 5px;min-height: 320px;background-color: #000000;color:#61688A;">
          <div style="height:32px;line-height:32px;padding-left:10px;border-bottom: 1px solid #27313e;font-size: 13px;">
            <span>{{$t("swap.myswapaccount")}}</span>
              <router-link class="linkmore" to="/swapAssets/">{{$t("swap.zijinhuazhuan")}}</router-link>
          </div>
        ...
        <!-- 开仓/平仓 -->
        <div class="order" style="margin-top: 5px;background-color: #000000;height: 320px;">
      ...
      <!-- 成交记录 -->
      <div v-if="isActiveMenu==3" class="left plate-wrap" style="position:relative;">
        <div style="background-color: #000000;height:40px;line-height:40px;padding-left:5px;color:#61688A;font-size: 13px;">
...
import expandRow from "@components/expand/swap.vue";
...
export default {
  components: { expandRow, DepthGraph, PageFooter, ContractTab},
  data() {
    let self = this;
    return {
      isShowSwapMenu:false,
      isActiveMenu:1,
      buyLeverage: "10",
      sellLeverage: "10",
        ...
        columns: [
          {
            ...
            render: (h, params) => {
              ...
              params.row.price != 0 && (price = params.row.price.toFixed(4)) && (str = h(
                  "span",
                  {
                    attrs: {
                      class: className
  ...
  computed: {
    ...
    positionList: function(){
      console.log("this.contractWalletInfo",this.contractWalletInfo);
      if(!this.contractWalletInfo || this.contractWalletInfo.id == 0 || this.currentPositionList.length == 0) {
        return;
      }
      var buyPl = 0;
      // 构建多仓数据
  ...
  methods: {
    ...
    setback() {
      ...
      !window.innerHeight && !doc.body.clientHeight && doc.documentElement.clientHeight && (height = doc.documentElement.clientHeight);
    ...
    getCNYRate() {
      this.$http
        .post(this.host + "/market/exchange-rate/usd-thb")
        .then(response => {
