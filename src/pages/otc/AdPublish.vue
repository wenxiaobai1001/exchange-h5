<template>
  <div class="page-home">
    <PageHeader :title="$t('uc.finance.billdetail')"></PageHeader> 
          ...
          <div class="">
            <div class="send-box">
              ...
              <div class="formbox send-form">
                <Form ref="form" label-position="top" :model="form" :rules="ruleValidate">
                  ...
                  <FormItem :label="$t('otc.publishad.country')" prop="country">
                    <Select v-model="form.country" @on-change="onAreaChange" :placeholder="$t('common.pleaseselect')">
                      <Option v-for="(area,index) in areas" :value="area.zhName" :key="index">{{area.enName}}</Option>
                    </Select>
                  </FormItem>
                  <!-- 1 -->
                  ...
                  <div class="ivu-form-item">
                    <label class="ivu-form-item-label">{{$t('otc.publishad.exchangeprice')}}</label>
                  ...
                  <!-- <router-link to="/uc/account" style="padding: 2% 0;color:#f0ac19;">{{$t('otc.publishad.tip2')}}</router-link> -->
                  <FormItem :label="$t('otc.publishad.paymode')" prop="payMode">
                    <Select v-model="form.payMode" multiple :placeholder="$t('common.pleaseselect')">
                      <Option v-for="(item,index) in payModeList" :value="item.value" :key="index" :disabled="item.isOpen">{{ item.label }}</Option>
                    </Select>
                  </FormItem>

                  <FormItem :label="$t('otc.publishad.minlimit')" prop="minLimit">
                    <Input v-model="form.minLimit" :placeholder="$t('otc.publishad.tip3')">
                    <span slot="append">(THB)</span>
                    </Input>
                  </FormItem>
                  ...
                  <FormItem :label="$t('otc.publishad.remark')" prop="remark" style="margin-top:12%;">
...
export default {
  ...
  data() {
    ...
    const maxCheck = (rule, value, callback) => {
      ...
      if (!value || value == 0) {
        ...
        callback(
          new Error(this.$t("otc.publishad.warning6") + priceNow + "(THB)！")
        );
      } else {
        callback();
      }
  ...
  methods: {
    changeCoin() {
      ...
      if (coinItem != null) {
        ...
        this.price = (cankoNew * lv).toLocaleString(); // + ' (THB)/' + coinItem.unit;
      ...
      this.$http
        ...
        .then(response => {
          ...
          if (resp.code == 0) {
            ...
            // this.price = resp.data.price + ' (THB)/' + resp.data.coinUnit;
  ...
  watch: {
    ...
    premisePriceHistory(newValue, oldValue) {
      ...
      // this.price = (cankoNew * lv).toLocaleString() + ' (THB)/' + this.findCoin(this.form.coin);
      // this.price = this.round(this.mul(cankoNew, lv), 2).toLocaleString() + ' (THB)/' + this.findCoin(this.form.coin);
