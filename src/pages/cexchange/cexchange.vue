                    ...
                    <div class="trade_bd_ctc">
                      <div class="panel panel_buy">
                        <div class="bd bd_limited">
                          <div style="min-height: 65px;width:100%;margin-bottom: 20px;padding: 5px;border-radius: 5px;background: #0b1520;">
                            <div style="width:50%;float:left;">
                              <h3>{{totalSupply}} CDEA</h3>
                              <p>{{$t('cexchange.restricted')}}</p>
                            </div>
                            <div style="width:50%;float:left;">
                              <h3>1 CDEA ≈ {{currentPriceShow}} {{baseCoinUnit}}</h3>
                              <p>{{$t('cexchange.currentprice')}}</p>
                            </div>
                          </div>
                          <Form style="width: 100%;" class="cexchange-trade">
                            <FormItem class="buy-input">
                              <div class="before" style="width:180px; text-align: left; font-size: 14px;">{{$t('cexchange.quantity')}}:</div>
                              <div class="quantity-group">
                                <div class="quantity-group-sub">
                                  <div :class="{ 'quantity-item': true, 'current': currentType == 1 }" @click="selectCount(1500, 1)"> 1500 CDEA </div>
                                  <div :class="{ 'quantity-item': true, 'current': currentType == 2 }" @click="selectCount(1000, 2)">1000 CDEA </div>
                                  <div :class="{ 'quantity-item': true, 'current': currentType == 3 }" @click="selectCount(500, 3)">500 CDEA </div>
                                  <div :class="{ 'quantity-item': true, 'current': currentType == 4 }" @click="selectCount(150, 4)">150 CDEA </div>
                                </div>
                                <div @click="selectCount(0, 5)" style="float:left;margin-top: 30px;margin-left:20px;">
                                  <Input style="width:200px;" v-model="inputAmount"  size="large" :placeholder="$t('cexchange.inputtips1')"></Input>
                                </div>
                              </div>
                            </FormItem>
                            <FormItem class="trade-input">
                              <div class="before" style="width:180px; text-align: left; font-size: 14px;">{{$t('cexchange.choosecurrency')}}:</div>
                              <RadioGroup v-model="baseCoinUnit">
                                  <div><Radio label="USDT" style="margin-bottom:10px;" >
                                    <span style="width:40px;display: inline-block;font-size: 14px;">USDT</span>
                                    <Input style="width:180px;" disabled :placeholder="$t('cexchange.assets') + assetUsdt"></Input>
                                    <Button style="width: 60px;margin-left:10px;" type="primary" @click="allBy(0)" size="small">{{$t('cexchange.all')}}</Button>
                                    <br/>
                                    <span style="margin-left: 20px;">1 USDT ≈ {{usdtRate}} CDEA </span>
                                  </Radio><br/></div>
                                  <div><Radio label="BTC" style="margin-bottom:10px;">
                                    <span style="width:40px;display: inline-block;font-size: 14px;">BTC</span>
                                    <Input style="width:180px;" disabled :placeholder="$t('cexchange.assets') + assetBtc"></Input>
                                    <Button style="width: 60px;margin-left:10px;" type="primary" @click="allBy(1)" size="small">{{$t('cexchange.all')}}</Button>
                                    <br/>
                                    <span style="margin-left: 20px;">1 BTC ≈ {{btcRate}} CDEA </span>
                                  </Radio><br/></div>
                                  <div><Radio label="ETH">
                                    <span style="width:40px;display: inline-block;font-size: 14px;">ETH</span>
                                    <Input style="width:180px;" disabled :placeholder="$t('cexchange.assets') + assetEth"></Input>
                                    <Button style="width: 60px;margin-left:10px;" type="primary" @click="allBy(2)" size="small">{{$t('cexchange.all')}}</Button>
                                    <br/>
                                    <span style="margin-left: 20px;">1 ETH ≈ {{ethRate}} CDEA </span>
                                  </Radio><br/></div>
                              </RadioGroup>
                            </FormItem>
                            <FormItem>
                              <div class="before" style="width:180px;text-align: left; font-size: 14px;">{{$t('cexchange.totalpay')}}:</div>
                              <div style="text-align:left;font-size:26px;font-weight: bold;color:#dc8e00;">{{totalPaymentShow | fixed8}}</div>
                            </FormItem>
                            <FormItem style="margin-top: 40px;" class="exchange-btn">
                              <Button v-if="isLogin" size="large" type="primary" @click="confirm()">{{$t('cexchange.confirmpay')}}</Button>
                              <Button v-else size="large" type="default">{{$t('cexchange.loginFirst')}}</Button>
                            </FormItem>
                          </Form>
                        </div>
                      </div>
...
import expandRow from "@components/expand/exchange.vue";
  ...
  computed: {
    ...
    currentPriceShow: function(){
      if(this.baseCoinUnit == "USDT") {
        return (1 / this.usdtRate).toFixed(2);
    ...
    getRate(){
      this.$http.post(this.host + this.api.market.thumb, {}).then(response => {
        ...
        for (var i = 0; i < resp.length; i++) {
          ...
          if(coin.symbol == "CDEA/USDT") {
            kwcPrice = coin.close;
          }
        }
        // 计算比例
        this.usdtRate = (1 / kwcPrice).toFixed(8);
        // this.btcRate = (kwcPrice / btcPrice ).toFixed(8);
        //this.ethRate = (kwcPrice / ethPrice).toFixed(8);
        this.btcRate = (btcPrice / kwcPrice ).toFixed(8);
        this.ethRate = (ethPrice / kwcPrice).toFixed(8);
      });
    },
    getAssets(){
      this.$http.post(this.host + this.api.uc.mywallet,{},{
        ...
        if (resp.code == 0) {
          ...
          for (let i = 0; i < this.tableMoney.length; i++) {
            ...
            if(this.tableMoney[i].coin.unit == this.platformCoin) {
              this.totalSupply = this.tableMoney[i].balance;
            }
          }
        } else {
...
.cexchange .notice-bottom{
  margin-top: 5px;
  height: 55px;background-color:#000000;padding-top: 12px;color: rgb(42, 147, 255);
}
