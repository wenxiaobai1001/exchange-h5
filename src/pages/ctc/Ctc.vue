<template>
  <div class="ctc">
    <PageHeader :headerType=0></PageHeader>
                ...
                <div class="trade_wrap">
                  <div class="trade_panel">
                    <div class="trade_bd_ctc">
                        ...
                        <div class="bd bd_limited">
                          <Form>
                            <FormItem class="buy-input">
                              ...
                              <label class="after" style="color: #45b854;">(THB)</label>
                            </FormItem>
                            ...
                            <div class="total buy_total" style="min-height">
                              <div style="min-height: 40px;">
                                ...
                                <div style="float:right;">
                                  ...
                                  <span style="font-size:14px;margin-left: 5px;color: #45b854;">(THB)</span>
                                </div>
                              </div>
                      ...
                      <p style="background: #e7e7f0;height: 2px;margin: 6% 0%;width: 100%;"></p>
                        ...
                        <div class="bd bd_limited">
                          <Form ref="formValidate">
                            <FormItem class="sell-input">
                              ...
                              <label class="after" style="color: #f2334f;">(THB)</label>
                            </FormItem>
                            <FormItem class="trade-input">
                            ...
                            <div class="total buy_total">
                              <div style="min-height: 40px;">
                                ...
                                <div style="float:right;color: #f2334f;">
                                  ...
                                  <span style="font-size:14px;margin-left: 5px;">(THB)</span>
                                </div>
                              </div>
                              <div style="width: 100%;font-size:12px;text-align:left;">{{$t("ctc.moneyTips")}}</div>
                            </div>
                    ...
                    <p style="background: #e7e7f0;height: 2px;margin: 6% 0%;width: 100%;"></p>
                    <div style="padding: 1% 5%;font-size: 12px;">
                      <div style="padding: 1%;width: 100%;text-align:left;line-height: 26px;">
                        ...
                        <p>{{$t("ctc.notice4")}}</p>
                        <p>{{$t("ctc.notice5")}}</p>
                        <!-- <router-link target="_blank" to="/helpdetail?cate=2&id=40&cateTitle=交易指南">{{$t("ctc.moredetail")}}</router-link> -->
                      </div>
    ...
    <Modal v-model="modal" width="450">
      ...
      <Form class="withdraw-form-inline" ref="formInline" :model="formInline" inline>
        ...
        <FormItem prop="code">
          <Input id="verifyCode" type="text" autocomplete="off" v-model="formInline.code" :placeholder="$t('uc.regist.smscode')"></Input>
    ...
    <Modal
        ...
        @on-ok="ok">
          ...
          <Row style="background: #27384a;padding: 10px 0px;border-radius: 5px;">
              ...
              <Col span="8">
                <p v-if="detailOrder.direction == 0" class="item-title green">{{detailOrder.money | toFixed(2)}} <span class="unit">(THB)</span></p>
                <p v-if="detailOrder.direction == 1" class="item-title red">{{detailOrder.money | toFixed(2)}} <span class="unit">(THB)</span></p>
                <p class="item-desc">交易总额</p>
              </Col>
          </Row>

          <div style="font-size: 12px;margin-top: 15px;" v-if="detailOrder.direction == 0">
            <Icon type="md-information-circle" style="color:rgb(183, 183, 183);margin-right:5px;font-size:14px;"/>请向以下收款账户汇款/转账： <span class="green" style="font-size: 20px;font-weight:bold;">{{detailOrder.money | toFixed(2)}}</span> <span class="green">(THB)</span>
          ...
          <div style="font-size: 12px;margin-top: 15px;" v-if="detailOrder.direction == 1">
            <Icon type="md-information-circle" style="color:rgb(183, 183, 183);margin-right:5px;font-size:14px;"/>你的以下账户将收到汇款/转账： <span class="red" style="font-size: 20px;font-weight:bold;">{{detailOrder.money | toFixed(2)}}</span> <span class="red">(THB)</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    getC2cPrice(){
      this.$http
        .post(this.host + "/market/ctc-usdt-thb")
        .then(response => {
          var resp = response.body;
    ...
    settime() {
      this.sendcodeValue = "will be sent again, after"+this.countdown+"s";
    ...
    ok() {
      if (this.formInline.code == "") {
        this.modal = true;
        this.$Message.error("Please input code");
        return;
      }
    ...
    valid(type){
      ...
      if(this.user.realVerified != 1){
        this.$Message.error("Please finish real-name authentication");
        return false;
      }
      if(this.user.fundsVerified != 1){
        this.$Message.error("Please set fund password");
        return false;
      }
      if(type == 0){
        if(this.buyAmount == "" || this.buyAmount == null || this.buyAmount == undefined){
          this.$Message.error("Please input buy amount");
          return false;
        }
        if(this.buyAmount < 50 || this.buyAmount > 50000) {
          this.$Message.error("Please input valid buy amount");
          return false;
        }
        return true;
      }else{
        ...
        if(this.sellAmount == "" || this.sellAmount == null || this.sellAmount == undefined){
          this.$Message.error("Please input sell amount");
          return false;
        }
        if(this.sellAmount < 50 || this.sellAmount > 50000) {
          this.$Message.error("Please input valid sell amount");
          return false;
        }
        return true;
      }
