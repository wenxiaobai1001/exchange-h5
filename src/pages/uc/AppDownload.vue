<template>
  <div class="login_form app-download">
    <div class="login_right">
      <div style="color: #000;margin-bottom: 60px;padding-top: 160px;width: 100%;text-align:center;z-index: 10;">
        ...
        <p style="font-size:18px;margin-top: 45px;">
            <span style="border: 1px solid #F90; padding: 8px 30px;border-radius: 5px;background-color:#F90;color:#FFF;" @click="downloadClick('0')">
            <Icon custom="i-icon iconfont iconupload-demo" style="font-size: 24px;margin-right: 5px;"/>Android</span>
            <span style="border: 1px solid #F90; padding: 8px 50px;border-radius: 5px;background-color:#F90;color:#FFF;" @click="downloadClick('1')">
    ...
    <div class="section" id="page4">
      <ul>
        <li>
          <div><img src="https://coineurglobalvip.oss-accelerate.aliyuncs.com/assets/images/download1.png" alt=""></div>
          <p class="title">Extreme Experience</p>
          <p>Meticulously optimized interface display, experience smooth operation response</p>
        </li>
        <li>
          <div><img src="https://coineurglobalvip.oss-accelerate.aliyuncs.com/assets/images/download2.png" alt=""></div>
          <p class="title">Currency Quotes</p>
          <p>Support MACD, KDJ, RSI, BOLL and other professional indicators</p>
        </li>
        <li>
          <div><img src="https://coineurglobalvip.oss-accelerate.aliyuncs.com/assets/images/download5.png" alt=""></div>
          <p class="title">Currency Transaction</p>
          <p>Support two methods of limit order and market order</p>
        </li>
        <li>
          <div><img src="https://coineurglobalvip.oss-accelerate.aliyuncs.com/assets/images/download3.png" alt=""></div>
          <p class="title">Fiat currency transaction</p>
          <p>Quality acceptors, to ensure the smooth flow of funds</p>
        </li>
        <li>
          <div><img src="https://coineurglobalvip.oss-accelerate.aliyuncs.com/assets/images/download4.png" alt=""></div>
          <p class="title">Asset Center</p>
          <p>Focus on asset changes anytime, anywhere, fast recharge/withdraw</p>
        </li>
      </ul>
    </div>
    <!-- <div class="cover" id="cover" @click="coverClick"><img src="https://coineurglobalvip.oss-accelerate.aliyuncs.com/assets/images/appdowncover.png"></div> -->
  </div>
</template>
...
export default {
  ...
  methods: {
    init() {
      window.document.title = (this.lang == "zh_CN" ? "APP下载 - " : "APP Download - ") + "CDEA | Global digital currency trading platform";
      this.getVersion();
    },
    downloadClick(platform){
      if(this.appURL){
        let url = "";
        for(let i =0; i<this.appURL.length;i++){
          if(platform==this.appURL[i].platform){
            url = this.appURL[i].downloadUrl;
          }
        }
        location.href = url;
      }
    ...
    getVersion(){
      let self = this;
      this.$http
        .post(this.host + "/uc/ancillary/system/get-all", null)
        .then(response => {
          var result = response.body;
          if (result.code == 0) {
            self.iosURL = result.data.downloadUrl;
            self.appURL = result.data;
          }
      });
      this.$http
