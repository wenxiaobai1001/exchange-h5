  ...
  <div class="option">
    ...
    <div class="page-content">
      <div class="ctc-container">
          <!-- <h1>{{$t('option.desc')}}</h1> -->
          <ContractTab contractUrl="/option"></ContractTab>
          <div class="main">
              <div class="option-tab">
                <div :class="{ 'option-tab-item': true, 'option-tab-current': item.symbol == currentCoinSymbol }" class="option-tab-item" v-for="(item, index) in coinList" @click="changeCoin(item.symbol)" :key="index">{{item.symbol}}</div>
              </div>
              <div class="ctc-container-main">
                      ...
                      <div class="panel panel_buy">
                        <div class="bd bd_limited">
                          <div class="result-panel">
                            <div style="width: 100%;font-weight: bold;font-size: 18px;text-align: center;margin-bottom: 15px;">
                            ...
                            <div v-show="historyTab==2" style="height: 350px;position: relative;">
                              <div id="kline_container_option"></div>
                            </div>
                            <div v-show="historyTab==1">
                              <div class="result-item" v-for="(item, index) in historyList" :key="index">
                          ...
                          <div class="last-period" v-if="openingOption.id != 0">
                            <div style="width: 100%;text-align: center;float:left;height:40px;line-height: 40px;font-size: 20px;font-weight: bold;text-shadow: 2px 2px 2px #000;">
                              {{$t('option.seriers')}} {{openingOption.optionNo}} {{$t('option.period')}}
                              <Tooltip placement="top-start">
                                <Icon type="ios-alarm" style="margin-top: -3px;"/>
                                <div slot="content"> {{(openingOption.createTime + currentCoin.openTimeGap * 1000) | dateFormat(that)}} ~ {{(openingOption.createTime + currentCoin.openTimeGap * 1000 + currentCoin.closeTimeGap * 1000) | dateFormat(that)}} </div>
                              </Tooltip>
                            </div>
                            <div class="period-content">
                              ...
                              <div class="content-item color_gold" v-if="isLogin">
                                ...
                                <span class="direction color_red" v-if="myOpeningOption.direction!='BUY'">{{$t('option.buydown')}}</span>
                                <!-- <span class="direction color_red" v-show="myOpeningOption.direction=='SELL'">{{$t('option.buydown')}}</span> -->
                              </div>
                            ...
                            <div style="color:#a5a5a5;width:170px;height:40px;text-align: center;">
                              <div style="height:16px;line-height: 16px;font-size: 12px;text-align: left;">{{$t('option.openprice')}}：{{openPrice | fixed2}} USDT</div>
                              <div style="height:16px;line-height: 16px;font-size: 12px;text-align: left;">{{$t('option.currentprice')}}：<span :class="{ 'color_green': currentPrice > openPrice, 'color_red': currentPrice < openPrice }">{{currentPrice | fixed2}} USDT</span>
                              </div>
                            </div>
                            <div  style="width:100%;height:40px;line-height: 40px;">
                              <div style="color:#ff8100;width:20%;height:40px;line-height: 40px;font-size:14px;">{{$t('option.progress')}}：</div>
                              <div style="width:80%;"><Progress :percent="currentPercent" :stroke-width="15" status="active"/></div>
                            </div>
                          </div>
                          ...
                          <Form style="width: 100%;margin-top: 20px;">
                            <FormItem class="buy-input">
                              <div class="quantity-group">
                                <div v-for="(item, index) in currentCoinAmountList" :key="index" :class="{ 'quantity-item': true, 'current': currentSelectCount == item }" @click="selectCount(item)"> {{item}} USDT</div>
                              </div>
                            </FormItem>
...
export default {
  components: { expandRow,PageHeader, PageFooter, ContractTab},
  data() {
    const self = this;
    return {
	  that:this,
  ...
  methods: {
    ...
    startKlineWebsock(){
      ...
      var that = this;
      var socket = new SockJS(that.wssHost + that.api.swap.ws);
    ...
    startWebsock() {
      ...
      var that = this;
      var socket = new SockJS(that.wssHost + that.api.option.ws);
    ...
    getKline() {
      ...
      let config = {
        ...
        toolbar_bg: "#0b1520",
        container_id: "kline_container_option",
...
<style lang="scss" scoped>
  ...
  .current-period{
    ...
    .period-time{
      display: inline-block;width: 100%;text-align: center;font-size: 14px;margin-bottom: 0px;letter-spacing:0px;
    }
    .period-reward{
