<template>
  <div class="page-home">
    <PageHeader :title="$t('uc.finance.withdraw.addressmanager')"></PageHeader>
    <div class="page-content">
      <div class="bill_box_address">

        <div class="action-inner">
          <div class="bill_box_address-item">
            <p class="describe">{{$t('uc.finance.withdraw.symbol')}}</p>
            <Select @on-change="changeCoin" v-model="coinType" style="width:250px;" size="large" :placeholder="$t('common.pleaseselect')">
              <Option v-for="item in coinList" :value="item.unit" :key="item.unit">{{ item.nameCn }}</Option>
            </Select>
          </div>
          <div class="bill_box_address-item">
            <p class="describe">{{$t('uc.finance.withdraw.address')}}</p>
            <div class="title">
              <Input v-model="withdrawAddr" style="width:250px;" size="large"></Input>
            </div>
          </div>
          <div class="bill_box_address-item">
            <p class="describe" v-if="currentCoin && currentCoin.accountType==1">{{$t('uc.finance.withdraw.receiverId')}}</p>
            <p class="describe" v-else>{{$t('uc.finance.withdraw.remark')}} / Memo</p>
            <div class="title">
              <Input v-model="remark" style="width:250px;" size="large"></Input>
            </div>
          </div>
        </div>

        <div class="bill_box_address-btn">
          <Button id="addrSubmit" @click='addAddr' size="large" style="width:150px;color:#fff;background:#f0a70a;border:1px solid #f0a70a;">{{$t('uc.finance.withdraw.add')}}</Button>
        </div>
        ...
        <div class="action-content">
          <div class="action-body">
            ...
            <div class="order-table">
              <Table :no-data-text="$t('common.nodata')" :columns="tableColumnsRecharge" :data="dataRecharge" :disabled-hover="true"></Table>
...
<script>
import expandRow from "@components/expand/withdrawAddress.vue";
...
export default {
    ...
    return {
      ...
      tableColumnsRecharge: [
        ...
        // },
        {
          ...
          render: (h, params) => {
            return h(
              "span",{}, params.row.unit+'-'+params.row.address
            );
          }
        },
        // {
  ...
  methods: {
    ...
    getCoin() {
      //币种
      this.$http.post(this.host + "/uc/withdraw/support/coinALL").then(response => {
        var resp = response.body;
        if (resp.code == 0) {
          for (let i = 0; i < resp.data.length; i++) {
            if(resp.data[i].unit!='USDT'){
    ...
    addAddr() {
      ...
      if (!this.coinType) {
        ...
        if(this.currentCoin.accountType==1){
          this.$Message.warning("Please input receiver ID");
        }else{
...
.page-home{
  ...
  .page-content{
    ...
    .bill_box_address{
      padding: 2%;
      .bill_box_address-btn {
        margin: 4%;
        text-align: center;
      }
      .bill_box_address-item{
        p.describe {
          font-size: 14px;
          font-weight: 600;
          padding: 2%;
